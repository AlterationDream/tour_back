        ...
        <div class="row">
            <div class="col-md-12">
                <nav class="breadcrumb push">
                    ...
                    <span class="breadcrumb-item active">Редактировать услугу</span>
                </nav>
                <!--<editor-fold desc="Service Create Form">-->
                <form action="{{ route('services.update', $service->id) }}" name="serviceCreate" id="serviceCreate"
                      method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="block block-rounded">
                        <!--<editor-fold desc="Block Header">-->
                        <div class="block-header block-header-default">
                            <h3 class="block-title">Редактировать услугу</h3>
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-12 mb-1 mb-xl-4 ">
                                            ...
                                            <input type="text" class="form-control" id="name"
                                                   name="name" placeholder="Название услуги" value="{{ old('name', $service->name) }}"
                                                   autocomplete="off">
                                        </div>
                                        <!--</editor-fold>-->
                                            ...
                                            <select id="category" name="category" class="js-select2" style="width: 100%;"
                                                ...
                                                @if(count($categories) > 0)
                                                    @foreach($categories as $category)
                                                        <option value="{{ $category->id }}" <?php if (old('category') == $category->id || $service->services_category_id == $category->id) echo 'selected';?>>{{ $category->name }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
                                        ...
                                        <div class="mb-1 mb-xl-4 col-sm-6 col-md-12 col-xl-6">
                                            ...
                                            <div class="input-group">
                                                <input type="text" class="form-control" id="price_value"
                                                       name="price_value" value="{{ old('price_value', $service->price_value) }}">
                                                ...
                                                if (old('price_currecny')) {
                                                    echo old('price_currecny');
                                                } else {
                                                    echo $service->currency_id;
                                                } ?>" autocomplete="off">
                                            ...
                                            <div class="input-group">
                                                <select name="price_unit" id="price_unit" class="form-select" autocomplete="off">
                                                    ...
                                                    @foreach($units as $unit)
                                                        <option value="{{ $unit->id }}" <?php if (old('price_unit') == $unit->id || $service->price_unit_id == $unit->id) echo 'selected';?>>{{ $unit->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                            ...
                                            <div class="input-group">
                                                <select name="country" id="country" class="form-select" autocomplete="off" onchange="fillCities()">
                                                    ...
                                                    @foreach($countries as $country)
                                                        <option value="{{ $country->id }}" <?php if (old('country') == $unit->id || $service->country_id == $unit->id) echo 'selected';?>>{{ $country->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-6">
                                            ...
                                            <div class="form-check form-switch">
                                                <input class="form-check-input" type="checkbox" value=""
                                                       ...
                                                       <?php if (old('active') || $service->active) echo 'checked'; ?> onchange="switchActive()">
                                            ...
                                            <div class="input-group">
                                                <select name="sale_tag" id="sale_tag" class="form-select"
                                                    ...
                                                    @foreach($saleTags as $saleTag)
                                                        <option value="{{ $saleTag->id }}" <?php if (old('sale_tag') == $saleTag->id || $service->sale_tag_id == $saleTag->id) echo 'selected';?>>{{ $saleTag->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
        ...
        <div class="modal" id="modal-delete" tabindex="-1" role="dialog" aria-labelledby="modal-small" aria-hidden="true">
            <div class="modal-dialog modal-sm" role="document">
                <div class="modal-content">
                    <form name="deleteForm" action="{{ route('services.delete', $service->id) }}" method="GET">
                        <div class="block block-rounded shadow-none mb-0">
                            <div class="block-header block-header-default">
                                <h3 class="block-title">Удаление услуги</h3>
                                <div class="block-options">
...
@section('js_after')
    ...
    <script>
        ...
        @if(old('country', $service->country_id))
            //console.log({{ old('city', $service->city_id) }})
            fillCities()
            @if(old('city', $service->city_id))
                $('#city').val({{ old('city', $service->city_id) }})
            @endif
        @endif
